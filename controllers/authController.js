...
const createSendToken = (user, statusCode, res) => {
    ...
    const jwtCookieExpiresIn = parseInt(process.env.JWT_COOKIE_EXPIRES_IN * 24 * 60 * 60 * 1000)
    const cookieOptions = {
...
exports.protect = catchAsync(async(req, res, next) => {
    // Get token and check if its exists
    let token;
    if(
    ...
    // Grant access to protectet route
    req.user = currentUser;
    // Wenn die User Seite nicht mehr lädt, die Zeile hier entfernen!
...
exports.restrictTo = (...roles) => {
    return (req, res, next) => {
        // Roles ['admin, 'lead-guide]. role = 'user -> not in array, no permission
